...
function CourseManagement() {
  ...
  const courseCategories = ['Arts and Humanities', 'Business', 'Information Technology', 'Language Learning', 'Personal Development', 'Social Sciences', 'Teaching and Academics'];
  ...
  const handleFileChange = (e) => {
    ...
    setEditData(prev => ({ ...prev, quizData: file }));
  };
        ...
        if (module.moduleId === moduleId) {
          return {
            ...
            subCourses: module.subCourses.map(sub =>
              sub.subCourseId === editing.subCourseId ? editData : sub
            )
          };
        }
        return module;
  ...
  const handleQuizUpload = async (e, courseId) => {
    setUploadError(null);
    setUploadSuccess(false);
    const file = e.target.files[0];
    if (!file) {
    ...
    try {
      ...
      reader.onload = (evt) => {
        try {
          ...
          setUploadSuccess("Quiz file processed successfully!");
        } catch (error) {
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="file"
                      accept=".json"
                      onChange={(e) => handleQuizUpload(e, course.id)}
